...
require_once (__DIR__ . "/../modelo/Sindico.class.php");
    ...
    class SindicoDAO {
        ...
        private function update(Morador $morador) {
            $sql = "UPDATE TB_MORADORES SET MOR_NOME ='{$morador->getNome()}', MOR_LOGIN ='{$morador->getLogin()}', MOR_SENHA ='{$morador->getSenha()}', MOR_ULTIMO_ACESSO ='{$morador->getUltimoAcesso()}', MOR_FOTO ='{$morador->getFoto()}', MOR_MOR_SIN ='{$morador->getFkMorSin()}' WHERE PK_MOR ='{$morador->getId()}'";
            try {
                Conexao::get()->exec($sql);
            } catch(PDOException $e) {
                echo $e->getMessage();
        ...
        private function update2(Morador $morador) {
            $sql = "UPDATE TB_MORADORES SET FK_MOR_SIN ='{$morador->getFkMorSin()}'";
            try {
                Conexao::get()->exec($sql);
            } catch(PDOException $e) {
                echo $e->getMessage();
